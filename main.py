...
def main():
    ...
    def draw_grid():
        ...
        grid_side = (
            grid_side - (grid_side % 8) + (grid_line_width * 7)
        )  # making sure grid_screen side length is divisible by 8 (plus the grid pixels)
    ...
    def place_shape():
        ...
        if chosen_shape != -1 and current_shapes[chosen_shape]:
            ...
            grid_pos_x, grid_pos_y = (curr_main_width / 2) - (
                grid_side / 2
            ), grid_padding
            square_side = (grid_side - grid_line_width * 7) / 8
            ...
            for i in range(8):
                for j in range(8):
                    current_pos = [
                        grid_pos_x + (j + 0.5) * square_side,
                        grid_pos_y + (i + 0.5) * square_side,
                    ]
                    distance = (current_pos[0] - mouse_x) ** 2 + (
                        current_pos[1] - mouse_y
                    ) ** 2
            ...
            for i in range(size[0]):
                for j in range(size[1]):
                    if shape.form[i][j] != 0:
                        pos_square = (
                            chosen_square[0] - size[0] + 1 + i,
                            chosen_square[1] - size[1] + 1 + j,
                        )
                        if (
                            pos_square[0] < 0
                            or pos_square[1] < 0
    ...
    def draw_shapes():
        ...
        if shape_animation_dist:
            shape_animation_dist = max(
                1, shape_animation_dist - shape_animation_velocity
            )
            if shape_animation_dist == 1:
                shape_animation_dist = 0
            shape_animation_velocity = shape_animation_dist / 10
    ...
    def draw_cursor():
        if chosen_shape != -1 and current_shapes[chosen_shape]:
            ...
            for i in range(size[0]):
                for j in range(size[1]):
                    if shape.form[i][j]:
                        pos_x = mouse_x - (square_side + grid_line_width) * (
                            size[1] + 0.5 - j
                        )
                        pos_y = mouse_y - (square_side + grid_line_width) * (
                            size[0] + 0.5 - i
                        )
                        square = pygame.Rect(pos_x, pos_y, square_side, square_side)
                        bg_square = pygame.Rect(
    ...
    def draw_score():
        ...
        text_highest_value = font_highest.render(
            str(highest_score), True, (135, 135, 135)
        )
